...
class Compile:
    ...
    def _create_entry_code(self, M: int, N: int, K: int, P: int, kTM: int, kTN: int,
                           kTK: int, kTP: int, warp_per_row: int, warp_per_col: int):
    ...
    def compile(self,
        ...
        file_name = (f"{self.file_prefix}_{M}_{N}_{K}_{P}"
                     f"_{kTM}_{kTN}_{kTK}_{kTP}_{warp_per_row}_{warp_per_col}")
        lib_path = os.path.join(temp_dir, f"{file_name}.so")

        if os.path.exists(lib_path):
            return lib_path

        entry_code = self._create_entry_code(M, N, K, P, kTM, kTN, kTK, kTP, 
